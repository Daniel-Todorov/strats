<template>
    <div class="language-switcher">
        <button class="active-language-button" @click="toggleExpand()">
            <span class="active-language-label">{{ $t('languageSwitcher.Language') }}</span>
        ...
        <div class="expandable absolute min-w-full" v-if="isExpanded">
            <button class="expanded-language-item w-full text-left" v-if="!isActiveLanguage(language)" v-for="language in languages" :key="language" @click="[switchLanguage(language), toggleExpand()]">{{ $t('languageSwitcher.Language', language) }}</button>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        data: function () {
            return {
                languages: process.env.VUE_APP_I18N_LOCALES.split(','),
                isExpanded: false
            }
        },
        methods: {
            ...
            switchLanguage(language) {
                ...
                this.$i18n.vm.locale = language;

                document.title = this.$i18n.t(title);
            },
            isActiveLanguage(language) {
                return this.$i18n.vm.locale === language
            },
            toggleExpand() {
